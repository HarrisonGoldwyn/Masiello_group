...
//	Reading in parameters. The parameter values are set in the "parameter list" file given as input to the program
	...
	float n_m = 0.0, lambda = 0.0, I_0 = 0.0, unit = 0.0; //n_m: background medium's index of refraction, lambda is the illuminating light's wavelength, I_0 is the intensity of the illuminating beam, and unit is the unit of length in nm that's used in the calculations
	...
	printf("%f %f %f %f %f %f\n", *kappa, *(kappa + 1), *(kappa + 2), lambda*1e6, n_m, I_0);
	printf("%d %d %d %d %d %d %d\n", d, x_min, x_max, y_min, y_max, z_min, z_max);
	printf("%d\n", input_mode);
	
//      Calculating Substrate Ratios
	float upper = (*(kappa + 2) - *kappa)/(*(kappa + 2) + *kappa);
	float lower = (*(kappa)*2)/(*(kappa + 2) + *kappa);
