...
!-----------------------------------------------------------------------
 ...
 7001 FORMAT(I10,' = number of dipoles in Target',/,                 &
             ...
             2I8,' = JYMIN,JYMAX',/,                                 &
             2I8,' = JZMIN,JZMAX',/,                                 &
             2F12.7,' = (x_TF/d)min,(x_TF/d)max',/,                  &
             2F12.7,' = (y_TF/d)min,(y_TF/d)max',/,                  &
             2F12.7,' = (z_TF/d)min,(z_TF/d)max',/,                  &
             2F12.7,' = xmin(TF),xmax(TF) (phys. units)',/,          &
             2F12.7,' = ymin(TF),ymax(TF) (phys. units)',/,          &
             2F12.7,' = zmin(TF),zmax(TF) (phys. units)',/,          &
             F12.7,' = d (phys units)',/,                            &
             2F12.7,' = PYD,PZD = period_y/dy, period_z/dz',/,       &
             2F12.7,' = period_y, period_z (phys. units)',/,         &  
             F12.7,' = wavelength in ambient medium (phys units)',/, &
             F12.7,' = k_x*d for incident wave',/,                   &
             F12.7,' = k_y*d for incident wave',/,                   &
             F12.7,' = k_z*d for incident wave',/,                   &
             1PE10.3,' = gamma (parameter for summation cutoff)')
 7002 FORMAT(0PF10.7,0PF10.7,' = (Re,Im)E_inc,x(0,0,0)',/,             &
             0PF10.7,0PF10.7,' = (Re,Im)E_inc,y(0,0,0)',/,             &
             0PF10.7,0PF10.7,' = (Re,Im)E_inc,z(0,0,0)',/,             &
...
7100 FORMAT(3F11.1,      &
 ...
 7112 FORMAT(3F11.1,      &
             ...
             E11.3,E11.3, &
             E11.3,E11.3 )
 7900 FORMAT('  err(E_x)=(',F10.7,',',F10.7,') ', &
               'err(E_y)=(',F10.7,',',F10.7,') ', &
               'err(E_z)=(',F10.7,',',F10.7,')')
 8002 FORMAT(0PF10.7,0PF10.7,' = (Re,Im)B_inc,x(0,0,0)',/,                 &
             0PF10.7,0PF10.7,' = (Re,Im)B_inc,y(0,0,0)',/,                 &
             0PF10.7,0PF10.7,' = (Re,Im)B_inc,z(0,0,0)',/,                 &
             4X,'x/d',6X,'y/d',6X,'z/d',5X,'----- B_x -----',              &
             3X,'----- B_y ------',3X,'----- B_z -----')
 9030 FORMAT('---------- physical extent in TF of target volume ---------',/, &
             2F14.7,' = (x_TF/d)min, (x_TF/d)max',/,                          &
             2F14.7,' = (y_TF/d)min, (y_TF/d)max',/,                          &
             2F14.7,' = (z_TF/d)min, (z_TF/d)max',/,                          &
             2F14.7,' = (x_TF)min,(x_TF)max (physical units)',/,              &
             2F14.7,' = (y_TF)min,(y_TF)max (physical units)',/,              &
             2F14.7,' = (z_TF)min,(z_TF)max (physical units)')
      END PROGRAM DDFIELD
