...
//	Reading in parameters. The parameter values are set in the "parameter list" file given as input to the program
	...
	float n_m = 0.0, lambda = 0.0, I_0 = 0.0, unit = 0.0, x_plane_min = 0.0, x_plane_max = 0.0; //n_m: background medium's index of refraction, lambda is the illuminating light's wavelength, I_0 is the intensity of the illuminating beam, and unit is the unit of length in nm that's used in the calculations
	...
	fscanf(par_id, "x_plane_min:  %f\n", &x_plane_min);
        fscanf(par_id, "x_plane_max:  %f\n", &x_plane_max);
	

	printf("%f %f %f %f %f\n", *kappa, *(kappa + 1), lambda*1e6, n_m, I_0);
...
//	Declaring useful variables
	...
	int N_init_guess = 600000; //Initial guess as to how many target points are in the input file
	...
	double *Q_ir = (double *)malloc(N_init_guess*sizeof(double)); //List of the corresponding heat source applied to each target point
	int x, y, z;
	int l, index, m_val;
	size_t n, p;
...
//	Reading in input file
	...
	while (!feof(inp_id)) {
		...
		*(Q_ir + N_init) = 4*M_PI*(2*M_PI*n_m/lambda)*I_0 * (unit*d) * (unit*d) * (unit*d) * (E_xr*P_xi - E_xi*P_xr + E_yr*P_yi - E_yi*P_yr + E_zr*P_zi - E_zi*P_zr) * (1e-18); //Calculates the heat source on each target point from the electric field and dipole moments at each lattice site (last factor is to convert W/m^3 to nW/nm^3) SCQ 7/26/17
		...
		*(Q_ir + N_init) -= 8*M_PI/3.0*(2*M_PI*n_m/lambda)*(2*M_PI*n_m/lambda)*(2*M_PI*n_m/lambda)*(2*M_PI*n_m/lambda)*I_0*(unit*d)*(unit*d)*(unit*d)*(unit*d)*(unit*d)*(unit*d)*(P_xr*P_xi + P_yr*P_yi + P_zr*P_zi)*(1e-45);
		...
		if (N_init >= N_init_guess) { //If the arrays have run out of room, then expand the memory allocated to them
			...
			Q_ir = (double *)realloc(Q_ir, 2*N_init_guess*sizeof(double));
			N_init_guess *= 2;
		}
	}
			...
			for (k = 0; k < num_z; k++) {
				...
				if (x_tar_min - 1 <= x && x <= x_tar_max + 1 && y_tar_min - 1 <= y && y <= y_tar_max + 1 && z_tar_min - 1 <= z && z <= z_tar_max + 1) { //We should check to see if the current point (given by x, y, z) is even close to the target. If not, there's no chance that this point is part of the extended target, so we shouldn't waste time checking for it.
					if (find_neighbor(initial_reading, N_init, k + (num_z + 1)*(j + (num_y + 1)*i)) < 0) { //If the current point (given by x, y, z) isn't in the list of target points from the input file...
						...
						} else if (x >= x_plane_min && x <= x_plane_max) { //See if the current point, although far away from the target, is still one that we'd like to know the temperature at. This should be manually changed to get the temperature at different outside points.
	...
	for (n = 0; n < N_bound; n++) {
		...
		*(D + n + N_bound*n) = 1.0;
		for (p = 0; p < N_bound; p++) {
			...
			*(D + n + N_bound*p) -= factor/(*kappa)*(*(G_r + abs(d_x + d - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)));
			...
			*(D + n + N_bound*p) -= factor/(*kappa)*(*(G_r + abs(d_x - s_x) + x_lim*abs(d_y + d - s_y) + x_lim*y_lim*abs(d_z - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)));
			...
			*(D + n + N_bound*p) -= factor/(*kappa)*(*(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z + d - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)));
		...
		*(Q_bound + n) = *(Q + N_in + n);
		for (l = 0; l < N_in; l++) {
			...
			factor = 2*(*(kappa + *(material + N_in + n)))*(*(kappa + *(neighbor_material + N_in + n + 0*N)))/(*(kappa + *(material + N_in + n)) + *(kappa + *(neighbor_material + N_in + n + 0*N))) - *kappa;
			*(Q_bound + n) += factor/(*(kappa + *(material + l)))*(*(G_r + abs(d_x + d - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)))*(*(Q + l));
			
			factor = 2*(*(kappa + *(material + N_in + n)))*(*(kappa + *(neighbor_material + N_in + n + 1*N)))/(*(kappa + *(material + N_in + n)) + *(kappa + *(neighbor_material + N_in + n + 1*N))) - *kappa;
			*(Q_bound + n) += factor/(*(kappa + *(material + l)))*(*(G_r + abs(d_x - d - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)))*(*(Q + l));
			
			factor = 2*(*(kappa + *(material + N_in + n)))*(*(kappa + *(neighbor_material + N_in + n + 2*N)))/(*(kappa + *(material + N_in + n)) + *(kappa + *(neighbor_material + N_in + n + 2*N))) - *kappa;
			*(Q_bound + n) += factor/(*(kappa + *(material + l)))*(*(G_r + abs(d_x - s_x) + x_lim*abs(d_y + d - s_y) + x_lim*y_lim*abs(d_z - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)))*(*(Q + l));
			
			factor = 2*(*(kappa + *(material + N_in + n)))*(*(kappa + *(neighbor_material + N_in + n + 3*N)))/(*(kappa + *(material + N_in + n)) + *(kappa + *(neighbor_material + N_in + n + 3*N))) - *kappa;
			*(Q_bound + n) += factor/(*(kappa + *(material + l)))*(*(G_r + abs(d_x - s_x) + x_lim*abs(d_y - d - s_y) + x_lim*y_lim*abs(d_z - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)))*(*(Q + l));
			
			factor = 2*(*(kappa + *(material + N_in + n)))*(*(kappa + *(neighbor_material + N_in + n + 4*N)))/(*(kappa + *(material + N_in + n)) + *(kappa + *(neighbor_material + N_in + n + 4*N))) - *kappa;
			*(Q_bound + n) += factor/(*(kappa + *(material + l)))*(*(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z + d - s_z)) - *(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)))*(*(Q + l));
...
//	Calculating and writing out temperatures
	...
	for (n = 0; n < N + N_out; n++) { //Iterate through every point at which we want to determine the temperature
		...
		for (l = 0; l < N; l++) { //Iterate through every point at which there is a heat source
			...
			*(T + n) += 1/((*kappa)*unit*d)*(*(G_r + abs(d_x - s_x) + x_lim*abs(d_y - s_y) + x_lim*y_lim*abs(d_z - s_z)))*(*(Q_eff + l)); //This source point's contribution to the temperature at (d_x, d_y, d_z) is simply its effective heat multiplied by the Green's function value that takes (s_x, s_y, s_z) to (d_x, d_y, d_z)
		}
