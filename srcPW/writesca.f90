...
!***
            IF(JPBC==0)THEN
               !Disallow negative values of Q NWB 7/25/13
               IF(QEXSUM(1).LT.0._WP)THEN
                  QEXSUM(1)=0._WP
               END IF
               IF(QABSUM(1).LT.0._WP)THEN
                  QABSUM(1)=0._WP
               END IF
               IF(QSCSUM(1).LT.0._WP)THEN
                  QSCSUM(1)=0._WP
               END IF
...
! Write orientationally-averaged Q values (except Qpha) to 'qtable':
            ...
            ENDDO
            !Disallow negative values of Q NWB 7/25/13
               IF(QAV(1).LT.0._WP)THEN
                  QAV(1)=0._WP
               END IF
               IF(QAV(2).LT.0._WP)THEN
                  QAV(2)=0._WP
               END IF
               IF(QAV(3).LT.0._WP)THEN
                  QAV(3)=0._WP
               END IF
