...
! --------------------------------- v6 ----------------------------
      ...
      INTEGER ::                                       &
         ...
         MXNAT,MODE,NAB,NAT0,NPT,NTHREADS,NX,NY,NZ,IC
      ...
      REAL(WP) ::                                           &
         ...
         IX, IY, IZ, EXR, EXI, EYR, EYI, EZR, EZI,          &
...
! Output files:
      ...
      OPEN(UNIT=224545, file='Pol.dat',    status='old')
      OPEN(UNIT=224646, file='tdda_input', status='replace')
...
! Modified by SCQ 31 July 2017
         IF(X(1)>-100._WP.AND.X(2)>-100._WP.AND.X(3)>-100._WP)THEN
            IF(REAL(CXE(1))>-10._WP.AND.IMAG(CXE(1))>-10._WP.AND. &
               ...
               WRITE(0,7100)X,CXE
               WRITE(7,7100)X,CXE
               READ(224545, *)  IC, PXR, PXI, PYR, PYI, PZR, PZI 
               WRITE(224646,'(3F7.1,I2,12F12.6)') X,IC, &
                               REAL(CXE(1)),AIMAG(CXE(1)),REAL(CXE(2)), &
                                AIMAG(CXE(2)), REAL(CXE(3)),AIMAG(CXE(3)), &
                                PXR, PXI, PYR, PYI, PZR, PZI
            ELSE
               WRITE(0,7110)X,CXE
               WRITE(7,7110)X,CXE
