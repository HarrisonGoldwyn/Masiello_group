...
! Arguments:
      ...
      INTEGER :: IBETH,IBETH1,IDVOUT,INIT,IORTH,IPBC,IPHI,IPHI1,ITASK, &
         ...
         NAT0,NAT3,NAVG,NCOMP,NSCAT,NTIMERS,NX,NY,NZ,idx
      ...
      REAL(WP) :: AK3,ETASCA,GAMMA,PIA2,TOL,TOLR,XMIN,YMIN,ZMIN,NBACK
...
! Local variables
      ...
      INTEGER :: NO_CG_RESTART, NO_CG_ITER, IX, IY, IZ
...
!     depend on direction of propagation and on polarization state)
            ...
            CALL ALPHADIAG(AKR,BETADF,PHIDF,THETADF,CALPHA,CXALPH,    &
                           ...
                           NY,NZ,CXRLOC,CSHAPE,SHPAR,NBACK)
...
!------------------------------------
               ...
               !IF (IWRPOL == 1) THEN
                  ...
                  DO i = 1, NAT0
                     WRITE(1093, *) IC(i), REAL(CXPOL(i + 0*NAT0)), IMAG(CXPOL(i + 0*NAT0)), &
                                    REAL(CXPOL(i + 1*NAT0)), IMAG(CXPOL(i + 1*NAT0)), &
                                    REAL(CXPOL(i + 2*NAT0)), IMAG(CXPOL(i + 2*NAT0))
                  ENDDO

                  OPEN(UNIT=1095, FILE='ddfield.in', STATUS='UNKNOWN')
                  WRITE(1095,*) 'w000r000k000.pol1     = name of file with stored polarization'
                  WRITE(1095,*) '5.00e-3 = gamma (interaction cutoff parameter)'

                  DO i = 1,NAT0
                     WRITE(1095, '(7I5)') FLOOR(IXYZ0(i,1)+XMIN-0.5), FLOOR(IXYZ0(i,2)+YMIN-0.5), &
                                          FLOOR(IXYZ0(i,3)+ZMIN-0.5), FLOOR(IXYZ0(i,1)+XMIN-0.5), &
                                          FLOOR(IXYZ0(i,2)+YMIN-0.5), FLOOR(IXYZ0(i,3)+ZMIN-0.5), 1
                  ENDDO
               !ENDIF
...
!------------------------------------
               ...
               ! Modified by NWB 29 Mar 2012
               DO i = 1, nat0
                  WRITE(100090, *) IXYZ0(i, 1), IXYZ0(i, 2), IXYZ0(i, 3)
               ENDDO
               CALL REDUCE(CXALPH,IOCC,MXN3,MXNAT,NAT,NAT0)

               DO i = 1, 3*NAT0
                  !WRITE(93, *) CXPOL(i), CXALPH(i)
                  WRITE(100093, *) REAL(CXPOL(i))
                  WRITE(100093, *) IMAG(CXPOL(i))
                  WRITE(100094, *) CXE(i)
               ENDDO
